...
//
// Vendored from Kronos (https://github.com/FCare/Kronos),
// yabause/src/core/video/opengl/common/include/6xbrz_shader.h, which carries the
// Yabause copyright and licence below. Kronos/Yabause is GPL-2.0-or-later, and the
// "or (at your option) any later version" grant is what makes it combinable with
// this project's GPL-3.0 licence. Keep this notice with the file.
//
...
//
// The GLSL is unmodified; it was only unpacked from the original escaped C string
// literals into raw string literals for legibility.
//
// Interface: attribute 0 = VertexCoord (clip space), attribute 1 = TexCoord.
// Uniforms: DrawingSize = source image size, TextureSize = allocated texture size
// (the shader uses texelFetch, so sampler filtering is irrelevant), Texture = unit 0.

#pragma once

namespace neonsaturn::android::shaders {

inline constexpr char kXbrz6xVertex[] = R"GLSL(
#version 310 es 
...
inline constexpr char kXbrz6xFragment[] = R"GLSL(
#version 310 es 
